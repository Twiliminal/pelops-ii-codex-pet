...
Output exactly 5 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Orange chibi shiba mascot, compact body, rounded head, cyan eyes, white muzzle and belly, red tongue patch at one cheek, cream bone collar across chest, curled tail, small blue patterned cloth tied behind the neck. Friendly desk companion personality..

Style contract: Codex digital pet sprite style: pixel-art-adjacent low-resolution mascot sprite, compact chibi proportions, chunky whole-body silhouette, thick dark 1-2 px outline, visible stepped/pixel edges, limited palette, flat cel shading with at most one small highlight and one shadow step, simple readable face, tiny limbs, and no detail that disappears at 192x208. Avoid polished illustration, painterly rendering, anime key art, 3D render, vector app-icon polish, glossy lighting, soft gradients, realistic fur or material texture, anti-aliased high-detail edges, and complex tiny accessories. Additional user style notes: Codex digital pet style: pixel-art-adjacent chibi sprite, chunky readable silhouette, thick dark blue 1-2 px outlines, limited flat palette, clean cel shading, tiny limbs, no text, no speech bubbles, no shadows, no loose effects..
...
Animation action: mouse-hover reaction using the standard action from `动态.png`. The middle frames must closely match the reference pose: head and body tilted diagonally, one eye squeezed shut in a wink, mouth/tongue expression visible, one paw raised high near the head, the other paw stretched lower, one leg lifted, and the blue cloth visible behind. This row is triggered by mouse hover in the Codex pet overlay.
...
- The held middle pose should feel paused briefly and trembling, not immediately pass through the action or switch into a different pose.
- Keep the wink/closed eye only while holding the standard action; avoid turning the whole row into a slow blink.
- Show the action through body tilt, wink, both paws lifted into the reference pose, lifted foot, and tiny pose jitter only.
- Do not draw ground shadows, contact shadows, drop shadows, oval shadows, landing marks, dust, smears, bounce pads, motion marks, wave marks, sparkles, symbols, or floating effects.
...
- Keep the dynamic pose readable, but do not crop the raised paw, ears, tail, neck cloth, bone collar, or lifted foot.
