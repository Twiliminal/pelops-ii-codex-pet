Create a single horizontal sprite strip for the Codex app digital pet `pelops-ii` in the state `idle`.
...
Animation action: default desktop idle with the existing subtle head motion preserved exactly: neutral stance, small head tilt, tiny paw/tail shift, quick small in-place shoulder/body turn, opposite head tilt, then return to neutral. Add only a short blink by changing the eyes in the middle frames.

Repair note from user: keep the current body/head/tail/paw/foot motion unchanged; only modify the eyes to add a blink. All 6 frames must keep the same head size, body height, belly size, bone-collar size, tail size, and foot baseline. The idle motion may include the existing gentle head tilt or quick small turn, but must not squash, shrink, stretch, zoom, crouch, or change the body proportions between frames.
...
- Keep all 6 frames nearly the same scale and vertical position; bounding boxes should stay visually stable with no sudden smaller frame.
