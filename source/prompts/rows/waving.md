...
Animation action: swapped old hover action: a small cheerful jump/bounce gesture, replacing the previous dynamic wink action in this row. Use the prior hover/jumping style as the semantic target: anticipation, small lift, bright raised-body pose, and return toward neutral.


State-specific requirements:
- This row has been swapped with the mouse-hover action. It should no longer use the `动态.png` wink pose.
- Show a compact jump/bounce action through pose and vertical body position only: neutral setup, anticipation, small lifted/cheerful pose, return toward neutral.
- Do not draw wave marks, motion arcs, lines, sparkles, symbols, floating effects, shadows, dust, landing marks, or floor cues.
...
- Keep the bounce pose readable, but do not crop the paws, ears, tail, neck cloth, bone collar, or feet.
