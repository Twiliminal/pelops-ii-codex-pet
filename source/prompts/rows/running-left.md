Create a single horizontal sprite strip for the Codex app digital pet `pelops-ii` in the state `running-left`.
...
Animation action: use the original `running` in-place run sequence for this left-run slot. This should match the older six-frame `running` loop style rather than a newly generated front-facing replacement.


State-specific requirements:
- This row must visually reuse the older `running` action sequence: compact forward-facing run-in-place, same face and stride pattern as the original row7 running animation.
- The Codex app expects 8 frames for this row, so preserve the old sequence first and only repeat frames from that same old sequence to fill the remaining slots.
- Do not make the pet travel toward the left, lean strongly sideways, or turn into a newly generated side-running pose.
...
- Keep all 8 frames centered and in place; no horizontal travel across the row.
