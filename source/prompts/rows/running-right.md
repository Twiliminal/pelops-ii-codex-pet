Create a single horizontal sprite strip for the Codex app digital pet `pelops-ii` in the state `running-right`.
...
Animation action: keep the previous rightward running locomotion. The pet should read as running toward the right, with a directional body lean and gait cycle.


State-specific requirements:
- This row remains the right-running row. Do not replace it with the front-facing in-place run.
- Use 8 frames of leg/arm bounce and body bob with readable rightward running intent.
- Preserve stable scale and slot centering; the body can lean right, but no pose may cross into a neighboring frame slot.
...
- Keep all 8 frames centered within their slots while reading as rightward running locomotion.
